from app.models import UsuarioCreate, UsuarioBase, ModeloPrediccionCreate, PruebaCreate
...
async def update_usuario(usuario_id: str, usuario_data: UsuarioBase) -> dict:
    await usuarios_collection.update_one({"_id": ObjectId(usuario_id)}, {"$set": usuario_data.dict()})
...
async def create_modelo(modelo_data: dict) -> dict:
    # Se espera que 'modelo_data' incluya los campos: nombre, descripcion, version, tipo_prueba, y archivo (ruta del archivo)
